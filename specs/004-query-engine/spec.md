...
- Q: Which membership roles may invoke query execution during Feature 4 delivery? → A: Any internal workspace role (`admin`, `analyst`, `viewer`); never `external_client` in this milestone.
...
Minimal internal-facing means to invoke execution for verification prior to reusable-saved-analysis and dashboard features; only **internal** workspace members (`admin`, `analyst`, `viewer`) may call execution—**`external_client` is barred** until a later milestone defines consumer orchestration with asset grants end-to-end.
...
- Direct query-engine execution by **`external_client`** members during this milestone; later work may expose permitted modalities only through layered flows that enforce explicit grants.

This specification still defines execution **modes** and identifiers those features will attach so integration does not reshape the engine contract later.
...
### User Story 3 - Cached answers where policy allows reuse (Priority: P3)

Repeated identical analytical requests that share cache eligibility may reuse cached tabular payloads only when caller consent and cache policy align. Cache hits must honor fresh authorization decisions and purge when underlying semantics change.
...
**Independent Test**: Two authorized requests with identical cache identity where policy allows caching produce first miss then hit with timing attributes; revocation or lowered permission denies serving cached payloads; eligibility differs by execution mode according to documented rules.

**Acceptance Scenarios**:

1. **Given** executions eligible under cache policy including saved-analysis and dashboard-tile modalities, **When** identical authoritative cache identity repeats within time-to-live, **Then** warehouse work may be skipped, metadata distinguishes cache utilization, authorization is revalidated at read time before returning cached rows, refusal downgrades reuse to guarded miss semantics.
2. **Given** eligible execution and later permission loss for the requesting principal, **When** reuse is attempted, **Then** refusal occurs without silently serving prior rows attributable to revoked access context.
3. **Given** a caller supplies an explicit bypass-reuse directive on an otherwise eligible invocation, **When** reuse would have occurred absent the directive, **Then** reuse is suppressed and invocation records reflect non-reuse semantics.
4. **Given** ad hoc exploratory execution mode policy, **When** identical submissions repeat, **Then** caching does not shorten warehouse work for that modality.
...
- **`external_client`** membership invokes execution: refusal with authorization categorical outcome prior to warehouse dispatch **without** requiring a synthesized passing authorization precondition upstream.

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: The system MUST accept execution requests differentiated by modality at minimum **ad hoc exploration**, **reusable saved analysis execution**, and **dashboard tile scoped execution**, including optional identifiers tying future owning assets without requiring those authoring surfaces to ship in this milestone.
- **FR-002**: Before any warehouse dispatch for a tenant-bound request, authentication context MUST resolve tenant, workspace, authenticated principal identifier, and membership role from the platform tenancy model. Principals holding **`external_client`** role MUST NEVER reach structural validation or warehouse dispatch for any modality in this milestone—the system MUST deny with categorical authorization refusal while still emitting immutable audit attribution where identifiers exist. For principals with internal workspace roles (**`admin`**, **`analyst`**, **`viewer`**), the system MUST require an explicit authorization precondition outcome for the requested modality (allowed vs denied) supplied by orchestrating layers; when denied or absent allowance, MUST short-circuit with authorization refusal BEFORE structural validation or warehouse touches while still producing immutable audit attribution where identifiers exist.
...
- **SC-003**: First eligible repeated dashboard-class or saved-analysis-class request pair within TTL shows reused prior results without another full computation on second invocation ≥80% of the time during a controlled regression harness while authorization revalidation remains enforced—**ad hoc modality never records reuse affirmative** in audits across harness repetitions unless eligibility policy expands.
...
- Query execution callers in this milestone are limited to **`admin`**, **`analyst`**, and **`viewer`**; **`external_client`** execution is deferred per clarification session above.
- Downstream authoring features supply canonical parameter declaration JSON shape later; MVP engine may ingest placeholder declarations for saved-analysis modality tests or stubs without full CRUD UX.
