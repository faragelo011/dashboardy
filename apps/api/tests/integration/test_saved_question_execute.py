"""Integration tests for saved-question execute delegation (US2)."""
...
def test_viewer_can_execute_with_valid_parameters(
    ...
    with TestClient(app) as client:
        executed = client.post(
            f"/workspaces/{seeded.workspace_id}/questions/{seeded.question_id}/execute",
            json={"parameters": {"region": "EMEA"}, "bypass_cache": False},
...
def test_execute_writes_audit_log_with_saved_question_id(
    use_live_postgres: None,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    seeded = asyncio.run(seed_question_catalog())
    ...
    with TestClient(app) as client:
        executed = client.post(
            f"/workspaces/{seeded.workspace_id}/questions/{seeded.question_id}/execute",
            json={"parameters": {"region": "NA"}, "bypass_cache": False},
            headers={"Authorization": "Bearer fake"},
        )
    assert executed.status_code == 200, executed.text
...
def test_execute_rejects_unknown_runtime_parameter(
    use_live_postgres: None,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    seeded = asyncio.run(seed_question_catalog())
    ...
    with TestClient(app) as client:
        executed = client.post(
            f"/workspaces/{seeded.workspace_id}/questions/{seeded.question_id}/execute",
            json={
                "parameters": {"region": "EMEA", "extra": "x"},
                "bypass_cache": False,
            },
            headers={"Authorization": "Bearer fake"},
        )
